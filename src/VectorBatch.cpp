...
void CompositeObject::AddObject(VectorObject* object, Vector2 position, float rotation, Color color)
{
...
VectorObject::VectorObject() : _thick(0), _width(0), _height(0), _solid(false), _x(0), _y(0), _s(1), _cx(0), _cy(0) { }
...
VectorObject* VectorObject::Regular(float r, int n)
{
    auto vo = new VectorObject();
    vo->moveTo(0, r);
    for(int i = 1; i < n + 1; i++)
...
void VectorBatch::DrawObject(CompositeObject* obj, Vector2 pos, float rot)
{
    rlPushMatrix();
    rlTranslatef(pos.x, pos.y, 0);
    rlRotatef(rot, 0, 0, 1);

    for(unsigned i = 0; i < obj->_objects.size(); i++)
    {
        DrawObject(obj->_objects[i], obj->_positions[i], obj->_rotations[i], obj->_colors[i]);
    }
